...
import { contactLinks, fullAddress, siteConfig } from "@/lib/site-config";
  ...
  return (
    <footer className="border-t border-aurora-earth/70 bg-[#f3eadc] px-4 py-10 sm:px-6 lg:px-8">
      <div className="container-shell flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="font-display text-[1.85rem] leading-none text-aurora-coffee">{siteConfig.name}</p>
          <p className="mt-2 inline-flex items-start gap-2 text-sm text-aurora-muted">
            <MapPin className="mt-0.5 h-4 w-4 shrink-0" />
            <span>{fullAddress}</span>
          </p>
          <p className="mt-2 text-sm text-aurora-muted">Instagram: {siteConfig.instagramHandle}</p>
        </div>
